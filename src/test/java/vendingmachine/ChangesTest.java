...
class ChangesTest {
    ...
    void _n원이_남아_있다면_n원_짜리_동전_1개_를_돌려_준다(int amount, CoinSet coin) {
        ...
        List<CoinSet> coinSet = changes.returnChanges();
    ...
    void 동전으로_잔돈을_거슬러_줄_수_없는_경우에도_최소_개수의_동전을_잔돈으로_반환한다() {
        ...
        coinSet = changes.returnChanges();
        assertThat(coinSet).containsExactlyInAnyOrder(CoinSet._500_COIN, CoinSet._100_COIN, CoinSet._50_COIN);
    }

    @Test
    void _500원이_남아있다면_반환_요청을_하는_경우_500원이_반환된다() {
        Changes changes = new Changes(500);
        assertThat(changes.returnChanges()).containsExactlyInAnyOrder(CoinSet._500_COIN);
    }

    @Test
    void _600원이_남아있다면_반환_요청을_하는_경우_500원_1개와_100원이_반환된다() {
        Changes changes = new Changes(600);
        assertThat(changes.returnChanges()).containsExactlyInAnyOrder(CoinSet._500_COIN, CoinSet._100_COIN);
    }

    @Test
    void _500원_동전이_들어오면_새로운_동전으로_반환되어야_한다() {
        Changes changes = new Changes(CoinSet._500_COIN.value);
        assertThat(changes.returnChanges()).isNotSameAs(List.of(CoinSet._500_COIN));
    }
}
